...
func test_melee_cleave_sweep_hits_targets_in_aim_vector() -> void:
	# Reset player and body transform to identity (unrotated) in global space
	_player.global_transform = Transform3D.IDENTITY
	_player._body.global_transform = Transform3D.IDENTITY
	_player.gravity = 0.0 # Disable gravity so player doesn't fall
	
	var enemy1: Enemy = _enemy
	enemy1.global_position = Vector3(0.0, 0.0, -1.5) # directly in front of body forward vector (negative Z)
	enemy1.collision_layer = 8 # Ensure it matches Enemy collision mask
	enemy1._is_dead = true # Set dead flag so enemy physics process returns early and doesn't fall
	...
	# Wait two physics frames to let Godot synchronize the new transforms to the physics server
	await get_tree().physics_frame
	await get_tree().physics_frame

	# Perform hit sweep check immediately
	_player.request_enemy_melee_hit()
	await get_tree().process_frame
	
	# Restore flags
	_player.gravity = ProjectSettings.get_setting("physics/3d/default_gravity")
	enemy1._is_dead = false
